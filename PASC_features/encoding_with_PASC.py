...
# and output an PASC_encoded_example_sequences.csv

# example command 'python encoding_with_PASC.py --f example_sequences.txt'
...
for i in range(len(three_lett_code)):
    l_aa_array=np.load('numpys/'+str(three_lett_code[i])+'_L_1024_0604.npy')
    d_aa_array=np.load('numpys/'+str(three_lett_code[i])+'_D_1024_0604.npy')
...
dataframe_X.to_csv('PASC_encoded_'+str(filename_no_extension)+'.csv')
