...
# and output an VOX_encoded_example_sequences.csv

# example command 'python encoding_with_VOX.py --f example_sequences.txt'
...
for i in range(len(three_lett_code)):
    l_aa_array=np.load('numpys/'+str(three_lett_code[i])+'_1573_L_0530.npy')
    d_aa_array=np.load('numpys/'+str(three_lett_code[i])+'_1573_D_0530.npy')
...
dataframe_X.to_csv('VOX_encoded_'+str(filename_no_extension)+'.csv')
